...
It shows how to provision a device, configure mesh models, and control the LEDs through a mobile application.
This sample can be used as a reference for creating custom Bluetooth Mesh applications that require LED control.
...
.. list-table:: Configuration files
   ...
   * - :file:`overlay-dfu.conf`
     - Enables DFU support in the application.
   * - :file:`sysbuild-dfu.conf`
     - Configuration file for :ref:`sysbuild <configuration_system_overview_sysbuild>` to enable DFU support.
   * - :file:`main.c`
     - Used to handle initialization.
   * - :file:`model_handler.c`
     - Used to handle mesh models.
...
Building and running DFU configuration
**************************************

In its DFU configuration, the sample uses the :file:`overlay-dfu.conf` and :file:`sysbuild-dfu.conf` files.

This configuration applies to the following board targets:

* ``nrf52840dk/nrf52840``
* ``nrf21540dk/nrf52840``
* ``nrf54l15dk/nrf54l15/cpuapp``

While this overlay configuration is only applicable for the mentioned board targets in this sample, DFU over Bluetooth Low Energy may be used on other platforms as well.
...
Configuring DFU configuration
=============================

|config|

To enable the DFU feature, set :makevar:`SB_CONF_FILE` to :file:`sysbuild-dfu.conf` and :makevar:`EXTRA_CONF_FILE` to :file:`overlay-dfu.conf` when building the sample.

Building DFU configuration
==========================
...
Building and running sample with FEM support
********************************************
