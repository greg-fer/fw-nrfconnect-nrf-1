...
While the applications use the same :ref:`nrf53_audio_app_overview_files`, you can select different :file:`*.conf` files to build them as either debug or release.
...
.. list-table:: Application versions
   ...
   * - Debug
     - :file:`prj.conf`
     - The debug application version is used for development and testing.
   * - Release
     - :file:`prj_release.conf`
     - | The release application version is used for production.
...
Additionally, the following configuration files are worth mentioning:
